...
class SinglePointPosition():
    ...
    def rescode(self, nav_list, OBS_DATA, rr, pos, err, dtr, V, Var, t_obs):
        ...
        for i in range(len(nav_list)):
            ...
            # dtrp = 0
            print("卫星号 = ", OBS_DATA.obsary[i].prn, "，接收机输出的伪距 =", pCorr[0], "  r=  ", r, "  ", " 接收机钟差 = ", dtr,
                  " 卫星钟差= ", nav_list[i].dts, " 电离层误差= ", ionCorr[0], " 对流层误差= ", tropCorr[0])
            ...
            print("r = ", r, "e = ", e)
            # nav_list[i].nav.prn,nav_list[i].nav.x,nav_list[i].nav.y,nav_list[i].nav.z,nav_list[i].dts,nav_list[i].vare,V[i],r)
            vn += 1
            s = SPPParm()
    ...
    def ionocorr(self, t_obs, pos, azel, iontype):
        ...
        if iontype == 1:
            ...
            if (pos[2] < -1E3 or azel[1] <= 0):
                ...
                ion = tool.CLIGHT * f * (
                    (5E-9 + amp * (1.0 + x * x * (-0.5 + x * x / 24.0))) if np.fabs(x) < 1.57 else 5E-9)
        var = ion * 0.5 * ion * 0.5
    ...
    def resdop(self, obs, nav, sol, vsat, azel, x):
        ...
        for i in range(len(obs.obsary)):
            lam = tool.lam[0]
            if obs.obsary[i].obsfrefList[0].D == 0.0 or lam == 0.0 or vsat[i] == 0 or tool.norm(nav[i].getVel(),
                                                                                                3) <= 0.0:
                continue
            ...
            rs = nav[i].getRS()
            rate = tool.dot_n(vs, [e[0][0], e[1][0], e[2][0]], 3) + (tool.OMGE / tool.CLIGHT) * (
                    rs[4] * sol.rr[0] + rs[1] * x[0] - rs[3] * sol.rr[1] - rs[0] * x[1])
